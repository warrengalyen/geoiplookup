...
func UpdateGeoLite2Country() {
	...
	db_update_url := fmt.Sprintf("https://download.maxmind.com/app/geoip_download?edition_id=GeoLite2-Country&license_key=%s&suffix=tar.gz", key)
...
func ExtractDatabaseFile(dst string, targz string) error {
	...
	Verbose(fmt.Sprintf("Extracting %s", targz))
